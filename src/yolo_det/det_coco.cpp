...
object_detect_result_list inference_yolo_coco_det_model(YoloV8_det model, cv::Mat input_image, bool enable_logger=false){
    ...
    if (ret != 0){
        std::cout << "inference_yolo_coco_det_model failed" << std::endl;
        result.count = -1;
        return result;
    }
