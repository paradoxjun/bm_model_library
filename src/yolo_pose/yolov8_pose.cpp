...
const std::vector<std::pair<int, int>> YoloV8_pose::pointLinks = {
    {15, 13}, {13, 11}, {16, 14}, {14, 12}, {11, 12}, {5, 11},
    {6, 12}, {5, 6}, {5, 7}, {6, 8}, {7, 9}, {8, 10}, {1, 2},
    {0, 1}, {0, 2}, {1, 3}, {2, 4}, {3, 5}, {4, 6}
};

bmcv_color_t YoloV8_pose::GetBmColor() {
    bmcv_color_t c;
...
int YoloV8_pose::Init(float confThresh, float nmsThresh) {
...
void YoloV8_pose::enableProfile(TimeStamp* ts) {
    m_ts = ts;
}

int YoloV8_pose::batch_size() {
    return max_batch;
};
...
int YoloV8_pose::pre_process(const std::vector<bm_image>& images) {
...
float YoloV8_pose::get_aspect_scaled_ratio(int src_w, int src_h, int dst_w, int dst_h, bool* pIsAligWidth) {
...
int YoloV8_pose::post_process(const std::vector<bm_image>& images, std::vector<poseBoxVec>& detected_boxes) {
    poseBoxVec yolobox_vec;
...
void YoloV8_pose::ProcessPoseBox(poseBoxVec& v, float* d, int n) {
    for (int i = 0; i < n; i++) {
...
void YoloV8_pose::ReTransPoseBox(poseBoxVec& v, float tx, float ty, float r, int fw, int fh, float* d, int n) {
    int i = 0;
    for (poseBox &b : v) {
        b.x1 = (b.x1 - tx) / r;
        b.y1 = (b.y1 - ty) / r;
...
void YoloV8_pose::NMS(poseBoxVec& dets, float nmsConfidence) {
    int length = dets.size();
    int index = length - 1;

    std::sort(dets.begin(), dets.end(), [](const poseBox& a, const poseBox& b) { return a.score < b.score; });
...
void YoloV8_pose::draw_bmcv(bm_handle_t& handle,
    poseBox& b,
    bm_image& frame,
    bool putScore) 
{
    ...
    for (auto& link : pointLinks) {
        ...
        bmPointsStart.push_back({
        static_cast<int>(b.keyPoints[link.first  * 3]),
        static_cast<int>(b.keyPoints[link.first  * 3 + 1])
        });

        bmPointsEnd.push_back({
            static_cast<int>(b.keyPoints[link.second * 3]),
            static_cast<int>(b.keyPoints[link.second * 3 + 1])
        });
