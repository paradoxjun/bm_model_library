...
object_pose_result_list inference_yolov8_kx_hp_pose_model(YoloV8_pose model, cv::Mat input_image, bool enable_logger){
    ...
    if (ret != 0){
        std::cout << "inference_yolov8_kx_hp_pose_model failed" << std::endl;
        result.count = -1;
        return result;
    }
    ...
    if (enable_logger) {
        ...
        for (int i = 0; i < result.count; ++i) {
            ...
            for (int k = 0; k < r.points_num; ++k) {
                ...
                // 点颜色：0123 红，45 黄
                cv::Scalar dot_col = (k < 4) ? cv::Scalar(0,0,255) : cv::Scalar(0,255,255);
                cv::circle(input_image, cv::Point(x,y), 3, dot_col, -1);
            }
        ...
        // ---- 保存 ----
        std::string img_file = "pose_kx_hp_result.jpg";
