...
void BYTETracker::update(STracks& output_stracks,
  ...
  if (result.count > 0) {
    for (int i = 0; i < result.count; i++) {
      ...
      tlbr_.resize(4);
      tlbr_[0] = result.results[i].box.left;
      tlbr_[1] = result.results[i].box.top;
      tlbr_[2] = result.results[i].box.right;
      tlbr_[3] = result.results[i].box.bottom;

      float score = result.results[i].prop;
      int class_id = result.results[i].cls_id;
