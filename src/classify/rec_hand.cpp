...
static const std::vector<std::string> ren_class_names = {
    "gp", "qm", "wsj", "gp_sz", "other"
};

/**
 * @brief  手势五分类（返回 Top‑1）
 */
cls_result inference_rec_hand_model(RESNET model, cv::Mat input_image, bool enable_logger = false){
    ...
    if (ret != 0 || vec.empty()) {
        cerr << "[rec_hand] Classify failed, ret=" << ret << endl;
        return { -1, 0.f };
    }
    ...
    if (enable_logger) {
        cout << "[rec_hand] class_id: " << class_id << " score: "  << score
