...
/**
 * @brief  款箱四分类（返回 Top‑1）
 */
cls_result inference_rec_kx_orient_model(RESNET model, cv::Mat input_image, bool enable_logger = false){
    ...
    if (ret != 0 || vec.empty()) {
        cerr << "[rec_kx_orient] Classify failed, ret=" << ret << endl;
        return { -1, 0.f };
    }
    ...
    if (enable_logger) {
        cout << "[rec_kx_orient] class_id: " << class_id << " score: "  << score
