...
#ifndef RESNET_NC_HPP
#define RESNET_NC_HPP
...
class RESNET_NC {
  ...
  int post_process(std::vector<bm_image> &images, std::vector<std::pair<int, int>> &results);
  static float get_aspect_scaled_ratio(int src_w, int src_h, int dst_w, int dst_h, bool *alignWidth);
  
  public:
  // RESNET_NC(std::shared_ptr<BMNNContext> context);
  RESNET_NC(std::shared_ptr<BMNNContext> context): m_bmContext(context){
    std::cout << "RESNET_NC()" << std::endl;
    //1. get network
    ...
    for (int i=0; i<output_num; i++){
      class_num = m_bmNetwork->outputTensor(i)->get_shape()->dims[1];
    ...
    // const std::vector<float> std = {1.0, 1.0, 1.0};
    // const std::vector<float> mean = {0.0, 0.0, 0.0};
  ...
  int batch_size();
  int Classify(std::vector<bm_image>& input_images, std::vector<std::pair<int, int>>& results);
};
